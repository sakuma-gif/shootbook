...
export default function App() {
  useEffect(() => {
    (async () => {
      try {
        const { data: sd } = await sb.from('staff').select('*');
        if (sd && sd.length > 0) setStaff([...sd.map(s=>({id:s.id,name:s.name,role:s.role,email:s.email||''})),...TEST_STAFF]);
        else { for(const s of REAL_STAFF) await sb.from('staff').upsert({id:s.id,name:s.name,role:s.role,email:s.email||''}); }
      } catch {}
      try {
        const { data: rd } = await sb.from('requests').select('*');
        if (rd) setReqs(rd.map(r=>({id:r.id,date:r.date,platforms:r.platforms||[],departments:r.departments||[],category:r.category,location:r.location,locationNote:r.location_note||'',amount:r.amount||0,amountCamera:r.amount_camera||0,amountHairmake:r.amount_hairmake||0,content:r.content,requester:r.requester,requesterEmpId:r.requester_emp_id||'',requesterSlackId:r.requester_slack_id||'',staffId:r.staff_id||'',staffIdHairmake:r.staff_id_hairmake||'',status:r.status||'pending',komban:r.komban||'',compo:r.compo||'',note:r.note||'',slackThreadUrl:r.slack_thread_url||'',eventUrl:r.event_url||''})));
      } catch {}
      try {
        const { data: nd } = await sb.from('ng_days').select('*');
        if (nd) setNgs(nd.map(n=>({staffId:n.staff_id,date:n.date})));
      } catch {}
      try {
        const { data: ed } = await sb.from('employees').select('*');
        if (ed && ed.length>0) setEmployees(ed.map(e=>({id:e.id,name:e.name,slackId:e.slack_id||''})));
      } catch {}
    })();
  }, []);
  ...
  const saveStaff = async v => {
    ...
    try { for(const s of withoutTest) await sb.from('staff').upsert({id:s.id,name:s.name,role:s.role,email:s.email||''}); } catch {}
  };
  const saveReqs = async v => {
    setReqs(v);
    try {
      const ids = v.map(r=>r.id);
      const {data:all} = await sb.from('requests').select('id');
      if(all) for(const row of all) if(!ids.includes(row.id)) await sb.from('requests').delete().eq('id',row.id);
      for(const r of v) await sb.from('requests').upsert({id:r.id,date:r.date,platforms:r.platforms||[],departments:r.departments||[],category:r.category,location:r.location,location_note:r.locationNote||'',amount:r.amount||0,amount_camera:r.amountCamera||0,amount_hairmake:r.amountHairmake||0,content:r.content,requester:r.requester,requester_emp_id:r.requesterEmpId||'',requester_slack_id:r.requesterSlackId||'',staff_id:r.staffId||'',staff_id_hairmake:r.staffIdHairmake||'',status:r.status||'pending',komban:r.komban||'',compo:r.compo||'',note:r.note||'',slack_thread_url:r.slackThreadUrl||'',event_url:r.eventUrl||''});
    } catch {}
  };
  const saveNgs = async v => {
    setNgs(v);
    try { await sb.from('ng_days').delete().neq('id',0); for(const n of v) await sb.from('ng_days').insert({staff_id:n.staffId,date:n.date}); } catch {}
  };
  const saveEmployees = async v => {
    setEmployees(v);
    try { for(const e of v) await sb.from('employees').upsert({id:e.id,name:e.name,slack_id:e.slackId||''}); } catch {}
  };
